...
<div class="card shadow-sm mb-4">
    <div class="card-body">
        ...
        <form method="post" action="<?= base_url('alumnos_carrera/filtrar') ?>" class="form-inline flex-wrap gap-2">
            ...
            <select name="codigo_carrera" id="codigo_carrera" class="form-control mb-2 mb-md-0 mr-md-2">
                ...
                <?php foreach ($carreras as $carrera): ?>
                    <option value="<?= esc($carrera['codigo_carrera']) ?>" <?= (isset($codigo_carrera_seleccionado) && $codigo_carrera_seleccionado === $carrera['codigo_carrera']) ? 'selected' : '' ?>>
                        <?= esc($carrera['codigo_carrera']) ?> - <?= esc($carrera['nombre_carrera']) ?>
                    </option>
                <?php endforeach; ?>
            </select>
    ...
    <div class="card-body">
        <?php if (empty($alumnos)): ?>
            <div class="text-center text-muted py-5">
                <?php if ($codigo_carrera_seleccionado): ?>
